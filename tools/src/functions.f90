...
function asep(l1,l2,b1,b2)   !Calculates the angular separation 
  ...
  implicit none
  real(double) :: asep,l1,l2,b1,b2,dl,db,b,rev2
  dl = rev2(l2-l1)
  db = rev2(b2-b1)
  b  = rev2((b1+b2)/2.d0)
...
!***********************************************************************************************************************************
function wdms(a1)    !Print angle as ddd.mm.ss string, input in rad with &deg; rather than d
  use SUFR_kinds, only: double
  use SUFR_constants, only: r2d
...
function wdms2(a1)   !Print angle as dms string, 
  !input in rad, output between -180 and +180 with &deg; rather than d
  use SUFR_kinds, only: double
  use SUFR_constants, only: r2d
...
!***********************************************************************************************************************************
function wdm(a1)       !Print angle as ddd:mm string, input in rad with &deg; rather than d
  use SUFR_kinds, only: double
  use SUFR_constants, only: r2d
...
function hms2rad(h,m,s) !Convert h:m:s to radians
  use SUFR_kinds, only: double
  use SUFR_constants, only: h2r
  
  implicit none
  real(double) :: hms2rad,h,m,s
  if(h.lt.0.d0) then
     m = -dabs(m)
     s = -dabs(s)
  end if
...
subroutine eq2gal(ra,dec,l,b) !Convert equatorial coordinates to 
  ...
  use SUFR_constants, only: 
  implicit none
  real(double) :: ra,dec,l,b,ra0,dec0,l0,rev
...
subroutine nutation(t,dpsi,eps0,deps)       ! Calculate nutation
  use SUFR_kinds, only: double
  use SUFR_constants, only: pi
  use constants, only: nutationdat
  
  implicit none
  real(double) :: tt,tt2,tt3,u,t
  real(double) :: d,ms,mm,f,o,dpsi,eps0,deps,tmp
  real(double) :: nu(9,63)
  integer :: i
...
subroutine readnutation     ! Read nutation input files
  use SUFR_kinds, only: double
  use SUFR_constants, only: 
  use constants, only: vsopdir, nutationdat
  ...
  do i=1,63
     read(10,'(5(I2),2x,I7,2x,F6.1,2x,I6,2x,F4.1)') nu1,nu2,nu3,nu4
...
function calcdeltat(jd)  !VERY SLOW, use calcdeltat1 if y,m,d are known
  ...
  use SUFR_constants, only: 
  implicit none
  real(double) :: calcdeltat,calcdeltat1,jd,d
  integer :: y,m
...
subroutine readdeltat
  use SUFR_constants, only: 
  use constants, only: vsopdir, deltatn,deltatnmax,deltatyrs,deltatvals
